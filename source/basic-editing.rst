...
movement keys. This toggles the current cell's wall in the selected direction
...
down the :kbd:`R` key and press one of the movement keys. This will use the
current special wall type, as indicated in the right-side tools pane. To select the
current special wall type, use the :kbd:`[` and :kbd:`]` keys.
...
In *Trail Mode*, the cursor will leave a trail behind as you move it around.
...
Use the :kbd:`T` key to toggle **Trail Mode**; you'll see two little
...
contents), press :kbd:`Ctrl+Alt+D`. Because of their destructiveness, these
two actions are undoable.
